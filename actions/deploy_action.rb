  ...
  class << self
    def call args, options
      ...
      if Code.has_changes? || options[:always_commit]
        cmds << "git add -A && git commit -a -m '#{SaveAction.commit_message(task_name, commit_message, options)}' #{Remote::Br.exists?(branch) ? " && git pull origin #{branch} || true " : nil} #{options[:no_push] ? nil : "&& git push origin #{branch}"}"
      end
